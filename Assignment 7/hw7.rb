...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    ...
    if inbetween.call(@x,seg.x1,seg.x2) and inbetween.call(@y,seg.y1,seg.y2)
      self
    else
      NoPoints.new
    end
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(@x1,@x2) # the segments are on a vertical line
      ...
      if real_close(aYend,bYstart) # just touching
        Point.new(aXend,aYend)
      elsif aYend < bYstart # disjoint
        NoPoints.new
      elsif aYend > bYend # b inside a
      ...
      if real_close(aXend,bXstart) # just touching
        Point.new(aXend,aYend)
      elsif aXend < bXstart # disjoint
        NoPoints.new
      elsif aXend > bXend # b inside a 
        LineSegment.new(bXstart,bYstart,bXend,bYend)
      else # overlapping
        LineSegment.new(bXstart,bYstart,aXend,aYend)
      end
